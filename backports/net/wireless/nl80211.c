...
void *backport_nl80211hdr_put(struct sk_buff *skb, u32 portid, u32 seq,
		     int flags, u8 cmd)
{
			      ...
			      int flags, struct nl80211_dump_wiphy_state *state)
{
	...
	hdr = backport_nl80211hdr_put(msg, portid, seq, flags, cmd);
	if (!hdr)
		return -ENOBUFS;
...
static int nl80211_get_key(struct sk_buff *skb, struct genl_info *info)
{
	...
	hdr = backport_nl80211hdr_put(msg, info->snd_portid, info->snd_seq, 0,
			     NL80211_CMD_NEW_KEY);
	if (!hdr)
		goto nla_put_failure;
				...
				struct mpath_info *pinfo)
{
	...
	hdr = backport_nl80211hdr_put(msg, portid, seq, flags, NL80211_CMD_NEW_MPATH);
	if (!hdr)
		return -1;
...
{
	void *hdr = backport_nl80211hdr_put(msg, NETLINK_CB(cb->skb).portid, seq, flags,
				   NL80211_CMD_GET_REG);
			    ...
			    struct cfg80211_internal_bss *intbss)
{
	...
	hdr = backport_nl80211hdr_put(msg, NETLINK_CB(cb->skb).portid, seq, flags,
			       ...
			       struct survey_info *survey)
{
	...
	hdr = backport_nl80211hdr_put(msg, portid, seq, flags,
			    ...
			    gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(skb, portid, seq, 0, cmd);
	if (!hdr) {
		kfree_skb(skb);
		return NULL;
	}
				 ...
				 struct netlink_callback *cb)
{
	...
	while (1) {
		void *hdr = backport_nl80211hdr_put(skb, NETLINK_CB(cb->skb).portid,
					   cb->nlh->nlmsg_seq, NLM_F_MULTI,
...
static int nl80211_connect(struct sk_buff *skb, struct genl_info *info)
{
	...
	pr_info("nl80211_connect: dev=%s iftype=%d pid=%d portid=%u",
		dev->name, dev->ieee80211_ptr->iftype,
		current->pid, info->snd_portid);

	if (!info->attrs[NL80211_ATTR_SSID] ||
	    !nla_len(info->attrs[NL80211_ATTR_SSID])) {
		pr_warn("nl80211_connect: reject missing SSID dev=%s", dev->name);
		return -EINVAL;
	}
	...
	if (dev->ieee80211_ptr->iftype != NL80211_IFTYPE_STATION &&
	    dev->ieee80211_ptr->iftype != NL80211_IFTYPE_P2P_CLIENT) {
		pr_warn("nl80211_connect: reject iftype=%d dev=%s",
			dev->ieee80211_ptr->iftype, dev->name);
		return -EOPNOTSUPP;
	}
	...
	pr_info("nl80211_connect: dev=%s ssid_len=%zu auth=%d privacy=%d",
		dev->name, connect.ssid_len, connect.auth_type,
		connect.privacy ? 1 : 0);
	...
	err = cfg80211_connect(rdev, dev, &connect, connkeys,
			       connect.prev_bssid);
	pr_info("nl80211_connect: cfg80211_connect returned err=%d dev=%s",
		err, dev->name);
	if (err)
		kfree_sensitive(connkeys);
			...
			struct cfg80211_nan_match_params *match, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_NAN_MATCH);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				  ...
				  u64 cookie, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_DEL_NAN_FUNCTION);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			    ...
			    struct genl_info *info)
{
	...
	u32 internal_flags;
	int err;
	u8 cmd = info->genlhdr ? info->genlhdr->cmd : 0;
	...
	case NL80211_CMD_DEAUTHENTICATE:
		pr_info("nl80211_pre_doit: cmd=%u ifname=%s portid=%u pid=%d",
			cmd, dev ? dev->name : "<none>", info->snd_portid,
			current->pid);
		break;
	default:
		break;
			    ...
			    struct cfg80211_sched_scan_request *req, u32 cmd)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, cmd);
	if (!hdr)
		return -1;
				     ...
				     struct regulatory_request *request)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, cmd_id);
	if (!hdr)
		goto nla_put_failure;
				 ...
				 gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_CONNECT);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			 ...
			 struct cfg80211_roam_info *info, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_ROAM);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				  ...
				  struct net_device *netdev, const u8 *bssid)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_PORT_AUTHORIZED);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			       ...
			       const u8 *ie, size_t ie_len, bool from_ap)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_DISCONNECT);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			     ...
			     gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_JOIN_IBSS);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
					...
					int sig_dbm, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_NEW_PEER_CANDIDATE);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				 ...
				 const u8 *tsc, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_MICHAEL_MIC_FAILURE);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				    ...
				    struct ieee80211_channel *channel_after)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_REG_BEACON_HINT);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			     ...
			     const u8 *ap_addr, u32 timeout)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_ASSOC_COMEBACK);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			  ...
			  gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_CONN_FAILED);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
		      ...
		      struct cfg80211_rx_info *info, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_FRAME);
	if (!hdr) {
		nlmsg_free(msg);
		return -ENOMEM;
	}
				    ...
				    gfp_t gfp, enum nl80211_commands command)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, command);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				     ...
				     bool unencrypted, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_CONTROL_PORT_FRAME);
	if (!hdr) {
		nlmsg_free(msg);
		return -ENOBUFS;
	}
					    ...
					    const char *mac, gfp_t gfp)
{
	...
	cb[0] = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_NOTIFY_CQM);
	if (!cb[0]) {
		nlmsg_free(msg);
		return NULL;
	}
				     ...
				     const u8 *replay_ctr, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_SET_REKEY_OFFLOAD);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			       ...
			       const u8 *bssid, bool preauth, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_PMKSA_CANDIDATE);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				     ...
				     u8 count, bool quiet)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, notif);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
		     ...
		     struct net_device *netdev, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_RADAR_DETECT);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				       ...
				       gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_STA_OPMODE_CHANGED);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
			   ...
			   bool is_valid_ack_signal, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_PROBE_CLIENT);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
				   ...
				   gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_SET_WOWLAN);
	if (!hdr)
		goto free_msg;
				...
				u16 reason_code, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_TDLS_OPER);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
		       ...
		       struct cfg80211_ft_event_params *ft_event)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_FT_EVENT);
	if (!hdr)
		goto out;
...
void cfg80211_crit_proto_stopped(struct wireless_dev *wdev, gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_CRIT_PROTOCOL_STOP);
	if (!hdr)
		goto nla_put_failure;
...
void nl80211_send_ap_stopped(struct wireless_dev *wdev)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_STOP_AP);
	if (!hdr)
		goto out;
				   ...
				   gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_EXTERNAL_AUTH);
	if (!hdr)
		goto nla_put_failure;
				    ...
				    gfp_t gfp)
{
	...
	hdr = backport_nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_UPDATE_OWE_INFO);
	if (!hdr)
		goto nla_put_failure;
...
int __init nl80211_init(void)
{
	...
	if (err)
		return err;
	pr_info("bp-nl80211 debug build active: pre_doit/connect tracing enabled");
